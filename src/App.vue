  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">设计风格</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">设计载体</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">UI/UX 美学设计与 AI 生图提示词指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - UI/UX与视觉设计需求专家');
...
const presetOptions = [
  "深色玻璃拟态 (Dark Glassmorphism) 移动端 APP",
  "Midjourney 3D 渲染科技风图标与插画提示词",
  "企业级 B 端管理后台仪表盘 UI 设计规范",
  "电商极简风首页 Banner 视觉排版与构图"
];
...
const metricsList = [
  {
    "key": "uiLayoutDesignRigor",
    "label": "UI布局设计严谨度"
  },
  {
    "key": "promptVisualExpressiveness",
    "label": "提示词视觉表现力"
  },
  {
    "key": "userExperienceUsability",
    "label": "用户体验易用性"
  },
  {
    "key": "brandIdentityConsistency",
    "label": "品牌视觉一致性"
  },
  {
    "key": "designSystemScalability",
    "label": "设计规范扩展度"
  }
];
const guideRules = [
  {
    "title": "对比度与视觉层级",
    "advice": "通过字号字重差异（如 24px 700 与 14px 400）拉开标题与正文层级，保证WCAG无障碍对比度",
    "avoid": "切忌全页面使用同一种字号与颜色造成信息混杂"
  },
  {
    "title": "组件网格对齐",
    "advice": "使用 8px 基础网格系统，内边距与外边距统一采用 8、16、24、32px 阶梯",
    "avoid": "避免使用没有规律的任意像素随意留白"
  },
  {
    "title": "AI 提示词结构化",
    "advice": "MJ 提示词按照“主体描述 + 场景环境 + 镜头构图 +  lighting光影 + 渲染引擎 + 参数”排列",
    "avoid": "切勿在提示词中使用抽象否定句或混乱堆砌近义词"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[SHEJI_SCORES\][\s\S]*?\[\/SHEJI_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[SHEJI_SCORES\](.*?)\[\/SHEJI_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('sheji_history_records');
...
const saveHistory = () => {
  localStorage.setItem('sheji_history_records', JSON.stringify(historyList.value));
};
